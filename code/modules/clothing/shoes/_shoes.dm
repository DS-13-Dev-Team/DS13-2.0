...
/obj/item/clothing/shoes/proc/handle_tying(mob/user)
	...
	if(user == loc && tied != SHOES_TIED) // if they're our own shoes, go tie-wards
		...
		if(do_after(user, our_guy, lace_time, extra_checks = CALLBACK(src, PROC_REF(still_shoed), our_guy)))
		...
		if(do_after(user, our_guy, mod_time, extra_checks = CALLBACK(src, PROC_REF(still_shoed), our_guy)))
